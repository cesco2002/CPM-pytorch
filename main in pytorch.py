...
def cell_interaction_energy(
    ...
    energy += torch.where(
        ...
        torch.logical_and(torch.tensor(x > 0), torch.tensor(cell_id != grid[x - 1, y])),
        J[cell_type, types_as_grid[x - 1, y]],
        0,
    )
    energy += torch.where(
        torch.logical_and(torch.tensor(x < grid.shape[0] - 1), torch.tensor(cell_id != grid[x + 1, y])),
        J[cell_type, types_as_grid[x + 1, y]],
        0,
    )
    energy += torch.where(
        torch.logical_and(torch.tensor(y > 0), torch.tensor(cell_id != grid[x, y - 1])),
        J[cell_type, types_as_grid[x, y - 1]],
        0,
    )
    energy += torch.where(
        torch.logical_and(torch.tensor(y < grid.shape[1] - 1), torch.tensor(cell_id != grid[x, y + 1])),
        J[cell_type, types_as_grid[x, y + 1]],
        0,
    )
    return energy
...
def propose_flip(
    ...
    while torch.all(neighbours == cell_id):
        x = torch.randint(size= (1,), low= 0, high = grid.shape[0])[0]
        #key, subkey = torch.random.split(key)
        y = torch.randint(size= (1,), low= 0, high = grid.shape[1])[0]

        cell_id = grid[x, y]
        # get surrounding cell types
        neighbours = torch.hstack(
            [
                grid[max(0, x - 1), y],
    ...
    u_neighbours = torch.tensor(u_neighbours)
    p = torch.tensor(p)
